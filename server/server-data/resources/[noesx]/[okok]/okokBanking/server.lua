...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Has depositado "..amount.."€", 5000, 'success')
	else
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Has retirado "..amount.."€", 5000, 'success')
	else
...
AddEventHandler("okokBanking:TransferMoney", function(amount, ibanNumber, targetIdentifier, acc, targetName)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
				    	...
				    	TriggerClientEvent('okokNotify:Alert', xPlayers[i], "BANK", "Has recibido "..amount.."€ from "..xPlayer.getName(), 5000, 'success')
				    end
				end
			...
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "No tienes tanto dinero en el banco.", 5000, 'error')
		end
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "No puedes enviarte dinero a ti mismo", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Has depositado "..amount.."€ to "..societyName, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "You don't have that much money on you", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			...
			TriggerClientEvent('okokNotify:Alert', _source, "BANK", "Has retirado "..amount.."€ from "..societyName, 5000, 'success')
		end
	else
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "IBAN cambiado exitosamente a "..iban, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Necesitas tener "..Config.IBANChangeCost.."€ in order to change your IBAN", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if Config.PINChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "PIN cambiado exitosamente a "..pin, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Necesitas tener "..Config.PINChangeCost.."€ in order to change your PIN", 5000, 'error')
	end
end)