...
Citizen.CreateThread(function()
    AddTextEntry('carRental', 'Alquiler de vehículos')
    AddTextEntry('carRentalClose', '~INPUT_PICKUP~ Alquiler de vehículos')
    while true do
        Wait(0)
        local letSleep = true
...
function OpenCarRental(index)
    ...
    while true do
        ...
        if WarMenu.Begin('carRental') then
            ...
            local _, totalTime = WarMenu.ComboBox('Tiempo de alquiler del vehículo', Config.RentalTimes, rentalTimeIndex)
            if rentalTimeIndex ~= totalTime then
                rentalTimeIndex = totalTime
            end

            if WarMenu.CheckBox('Precio - '..Config.InsurancePrice..'$', insurance) then
            ...
            if WarMenu.Button('Alquilar Vehículo', price..'$') then
                if DoesEntityExist(lastLocalVehicle) then
                    DeleteEntity(lastLocalVehicle)
                    lastLocalVehicle = nil
                    FreezeEntityPosition(PlayerPedId(), false)
...
function startTimer()
    Citizen.CreateThread(function()
        ...
        while rentalTimer>0 do
            ...
            AddTextComponentSubstringPlayerName(disp_time(rentalTimer).." - Alquiler Tiempo restante")
...
Citizen.CreateThread(function()
    ...
    AddTextEntry('carReturnClose', '~INPUT_PICKUP~ Devolver el vehiculo')
    while true do
        Wait(0)
        if rentalTimer > 0 then
            local letSleep = true
...
AddEventHandler('sqz_carrental:VehicleSuccessfulyReturned', function()
	...
	PushScaleformMovieMethodParameterString('El vehículo ha sido devuelto exitosamente.')
