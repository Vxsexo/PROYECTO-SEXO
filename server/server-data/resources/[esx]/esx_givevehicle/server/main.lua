...
-- GIVEAR ITEMS A TODOS EL SERVIDOR

RegisterCommand('giveitemall', function(src, args)
    local src = src 
    ...
    if group == 'admin' or group == 'superadmin' then 
        local item = args[1]
        if not item then
            xPlayer.showNotification('Debes especificar un item')
            return
        end
        local amount = args[2]
        if not amount then
            xPlayer.showNotification('Debes especificar una cantidad')
            return
        end
        if not tonumber(amount) then
            xPlayer.showNotification('La cantidad debe ser un número')
            return
        end
        local players = ESX.GetExtendedPlayers()
        for k,v in pairs(players) do
            v.addInventoryItem(item, amount)
            v.showNotification('Se te ha dado ' .. amount .. ' ' .. item)
        end
    else
