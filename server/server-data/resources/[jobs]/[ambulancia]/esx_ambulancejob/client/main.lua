...
function RemoveItemsAfterRPDeath()
	...
	local havePoint = Config.RespawnPointsJob[ESX.PlayerData.job.name]
	if not havePoint then 
		...
		local elements = {}
		table.insert(elements, {label = havePoint.label, value = ESX.PlayerData.job.name, coords = havePoint.coords})
		table.insert(elements, {label = 'Hospital', value = 'hospital', coords = Config.RespawnPoint.coords})
		ESX.UI.Menu.CloseAll()
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'respawn_point', {
			title = 'Selecciona un punto de respawn',
			align = 'right',
			elements = elements
		}, function(data, menu)
			CreateThread(function()
				...
				ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
					local formattedCoords = {
						x = data.current.coords.x,
						y = data.current.coords.y,
						z = data.current.coords.z
					}
