    ...
    while true do
        if isBusDriver and not isRouteJustAborted then
            ...
            elseif isPlayerNotInBus() then
                ...
                ESX.ShowFloatingHelpNotification(_U("get_back_in_bus"), vector3(playerX, playerY, playerZ))
...
function handleSpawnPoint(locationIndex)
    ...
    if playerDistanceFromCoords(coords) < Config.Markers.Size then
		local playerPed = PlayerPedId()
        local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
        ESX.ShowFloatingHelpNotification(_U("start_route", route.Name), vector3(playerX, playerY, playerZ))
...
function handleAbortRoute()
    if playerDistanceFromCoords(activeRoute.SpawnPoint) < Config.Markers.Size then
        local playerPed = PlayerPedId()
        local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
        ESX.ShowFloatingHelpNotification(_U("abort_route_help", totalMoneyPaidThisRoute), vector3(playerX, playerY, playerZ))
